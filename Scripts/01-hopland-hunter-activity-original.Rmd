...
setwd("C:/Users/Alex/Dropbox/Berkeley_Classes/Dissertation/Hunting/R/R_for_Alex")
...
# get collar files
wd <- getwd()
collarfiles <- paste(wd, "/Data/Hunting/igotu_raw/", dir("./Data/Hunting/igotu_raw"), sep="")
...
filenames <- list.files(path = "C:/Users/Alex/Dropbox/Berkeley_Classes/Dissertation/Hunting/R/R_for_Alex/Data/Hunting/igotu_raw/", pattern = NULL, all.files = FALSE, full.names = TRUE, recursive = FALSE, ignore.case = FALSE)
...
#Alex: use above code instead of this line
#hunter.csvs <- lapply(collarfiles, FUN=read.csv)
...
# an alternative function that will also subset to points every 10 minutes
# points are every 5 seconds, so 20 per min, or 200 per 10 min
#ExtractCollarData2 <- function (X){
#  X <- X[seq(1, NROW(X), by = 200),] # take every 200th row
#  id <- X[,"Source"]
#  unit.time <- X[,"Time"]
#  unit.date <- X[,"Date"]
#  unit.datetime <- paste(unit.date, unit.time, sep="")
...
#extracted.list.10min <- lapply(hunter.csvs, FUN=ExtractCollarData2)
...
#spdf.list.10min <- lapply(extracted.list.10min, FUN=SPDFTransform)

# combine list into single SPDF 
spdf <- do.call("rbind", spdf.list)
spdf.wgs <- do.call("rbind", spdf.list.wgs)

#spdf.10min <- do.call("rbind", spdf.list.10min)
#plot(spdf.10min)
...
plotltr(hunter.ltraj, "dt")
#Alex: hm lots of errors with above line. Ignoring for now. Line below rediscretizes by time to 10min intervals, up from 5s
...
df.spdf <- single.df
coordinates(df.spdf) <- ~lon+lat # creat spdf
