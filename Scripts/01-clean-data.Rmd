...
filter to 2 individuals
```{r}
igotu_data_all <- igotu_data_all %>% 
  filter(ID %in% c("080815_01", "081917_29", "080915_11"))
```
...
hunter_resampled <- hunter_tracks %>%
  ...
  mutate(resampled_data = map(data, function(x)
    x %>% track_resample(rate = minutes(10),
                         tolerance = minutes(2)))) %>%
...
igotu_data_10min <- left_join(hunter_resampled, igotu_data_cropped) 

write.csv(igotu_data_10min, "Data/igotu_data_10min.csv", row.names=F)
```
