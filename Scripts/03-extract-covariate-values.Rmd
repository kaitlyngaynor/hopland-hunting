...
```{r}
# remove points outside of huntable zone
huntable <- read_sf("Data/Spatial data/Raw from Alex/hunt_zone.shp") %>% 
    st_transform("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

igotu_data_cropped <- igotu_data[st_intersects(igotu_data, huntable) %>% lengths > 0,]
```
