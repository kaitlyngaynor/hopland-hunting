...
library(ggplot2)
igotu_cropped_plots <- lapply(sort(unique(igotu_data_cropped$ID)), function(i) {
    
    ggplot() + 
        geom_sf(data = huntable_sf) +
        geom_sf(data = igotu_data_cropped[igotu_data_cropped$ID==i,]) +
        theme_bw() +
        ggtitle(i) 
})

# print all plots to pdf
pdf("igotu_cropped-plots-newtracks.pdf")
for (i in 1:length(igotu_cropped_plots)) {
    print(igotu_cropped_plots[[i]])
}
dev.off()
```
