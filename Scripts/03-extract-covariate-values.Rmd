...
```{r}
igotu_data_cropped <- read.csv("Data/igotu_data_cropped.csv")

# convert coordinates to spatial points
igotu_data_cropped.sp <- igotu_data_cropped %>% 
...
```{r raster extract}
igotu.raster <- as.data.frame(raster::extract(raster.stack, igotu_data_cropped.sp)) %>% 
...
```{r combine metadata}
igotu_data_cropped_covariates <- cbind(igotu_data_cropped, igotu.raster) 
```

Export
```{r}
write.csv(igotu_data_cropped_covariates, "Data/igotu_data_cropped_covariates.csv")
```
